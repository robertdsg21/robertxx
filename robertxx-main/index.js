...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: ROBERT DESIGN\n'//GANTI NAMA LU COK
            + 'ORG:Criador ROBERT;\n'//GANTI NAMA LU!!
            + 'TEL;type=CELL;type=VOICE;waid=5571999926079:+55 71 99992-6079\n'//GANTI NOMOR LU
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SALVEEE @${num.split('@')[0]}\nBem vindo Ao Grupo*${mdata.subject}*\n*FIQUE A VONTADE :)*\n\n*LEIA AS REGRAS PRA NN SER REMOVIDO(A):D*`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["5571999926079@s.whatsapp.net"]
			...
			const ownerNumber = [
			"5571999926079@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"5571999926079@s.whatsapp.net","6283102650464@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
...
/*********END PISAH MENU***********/
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'leave': 
	                ...
	                break
	            case 'getses':
                    if (!isOwner) return client.reply(from, 'Este comando é apenas para o Robert', id)
         ...
         case 'lirik':
                ...
                case 'report':
                    ...
                    reply('Problemas foram relatados ao Robert, relatórios falsos não serão respondidos..')
                    break
              case 'request':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('5571999926079@s.whatsapp.net', options, text, {quoted: mek})
                    reply('As solicitações foram relatadas ao Robert, as solicitações que podem sobrecarregar o proprietário não serão respondidas.')
                    break
				case 'meme':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	   ...
	   case 'coli':
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 ROBERT DESIGN〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
